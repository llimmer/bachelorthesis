...
impl IPS2RaSorterDMA {
    fn calculate_pointers(&mut self, task: &DMATask) {
        self.boundaries[0] = 0;
        self.pointers[0].0 = 0;
        let mut sum = 0;
    ...
    pub fn permutate_blocks(&mut self, task: &mut DMATask) {
        self.calculate_pointers(task);
        let mut pb: usize = self.primary_bucket;
        ...
        'outer: loop {
            ...
            // TODO: check if already in right bucket and read < write, skip in this case
            // TODO: keep track of loaded hugepages, only write/read if not in memory

            let idx = self.pointers[pb].1 + BLOCKSIZE as i64;
            let (hugepage, chunk, block) = calculate_hugepage_chunk_block(idx as usize);

            // read chunk
            lba[swap_buffer_idx] = hugepage*CHUNKS_PER_HUGE_PAGE*LBA_PER_CHUNK+chunk*LBA_PER_CHUNK;
            self.qpair.submit_io(&mut self.dma_blocks[swap_buffer_idx].slice(0..CHUNK_SIZE), lba[swap_buffer_idx] as u64, false);
            self.qpair.complete_io(1);

            // read block into swap buffer
            swap_buffer[swap_buffer_idx].copy_from_slice(&self.dma_blocks[0][block*BLOCKSIZE*8..(block+1)*BLOCKSIZE*8]);
            ...
            'inner: loop {
                ...
                *wdest += BLOCKSIZE as i64;
                ...
                // read chunk
                lba[next_swap_buffer_idx] = next_hugepage*CHUNKS_PER_HUGE_PAGE*LBA_PER_CHUNK+next_chunk*LBA_PER_CHUNK;
                self.qpair.submit_io(&mut self.dma_blocks[next_swap_buffer_idx].slice(0..CHUNK_SIZE), lba[next_swap_buffer_idx] as u64, false);
                self.qpair.complete_io(1);


                if *wdest-BLOCKSIZE as i64 <= *rdest {
                    // copy to new swap buffer
                    swap_buffer[next_swap_buffer_idx].copy_from_slice(&self.dma_blocks[next_swap_buffer_idx][next_block*BLOCKSIZE*8..(next_block+1)*BLOCKSIZE*8]);
                    // overwrite with old swap buffer
                    &self.dma_blocks[next_swap_buffer_idx][next_block*BLOCKSIZE*8..(next_block+1)*BLOCKSIZE*8].copy_from_slice(&swap_buffer[swap_buffer_idx]);

                    // write back to disk
                    println!("writing {:?} to lba {}", u8_to_u64_slice(&mut swap_buffer[swap_buffer_idx]), lba[next_swap_buffer_idx]);
                    self.qpair.submit_io(&mut self.dma_blocks[next_swap_buffer_idx].slice(0..CHUNK_SIZE), lba[next_swap_buffer_idx] as u64, true);
                    self.qpair.complete_io(1);
