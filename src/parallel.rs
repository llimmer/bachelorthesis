...
pub(crate) fn sort_parallel(task_queue: Arc<Mutex<VecDeque<Box<IPS2RaSorter>>>>,
...
//
    ...
    //            let mut new_struct = IPS2RaSorter::new_parallel(current);
    //            {
    //                let mut queue = task_queue.lock().unwrap();
    ...
    //        {
    //            let mut queue = task_queue.lock().unwrap();
    //            queue.push_back(IPS2RaSorter::new_parallel(all));
    //        }
//
//
    //        task_counter.fetch_sub(1, Ordering::SeqCst);
