...
fn qd1(
    ...
    let mut buffer: Dma<u8> = Dma::allocate(HUGE_PAGE_SIZE_2M)?;
...
fn qd_n(
    ...
    for i in 0..n_threads {
        ...
        let handle = thread::spawn(move || -> (u64, f64) {
            ...
            let mut buffer: Dma<u8> = Dma::allocate(HUGE_PAGE_SIZE_2M).unwrap();
...
fn fill_ns(nvme: &mut NvmeDevice) {
    let buffer: Dma<u8> = Dma::allocate(HUGE_PAGE_SIZE_2M).unwrap();
